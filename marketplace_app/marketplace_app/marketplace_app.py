...
def listing_card(listing: Listing) -> rx.Component:
    return rx.box(
        rx.vstack(
            rx.hstack(
                rx.badge(f"Token #{listing.token_id}", color_scheme="green", variant="surface"),
                rx.spacer(),
                rx.text(listing.price, font_weight="800", font_size="1.1rem", color="white"),
                width="100%",
                align_items="center",
            ),
            rx.text(f"{listing.amount} tonnes available", color=TEXT_MUTED, font_size="0.9rem"),
            rx.divider(margin_y="0.5em", opacity="0.2"),
            rx.hstack(
                rx.text("Seller", color="gray", font_size="0.8rem"),
                rx.spacer(),
                rx.text(f"{listing.seller[:6]}...{listing.seller[-4:]}", color="white", font_size="0.8rem", font_family="monospace"),
                width="100%",
            ),
            rx.button(
                "Execute trade",
                ...
                _hover={"transform": "translateY(-2px)", "box_shadow": f"0 5px 15px {ACCENT}40"},
                transition="all 0.2s",
            ),
            spacing="2",
        ),
        ...
        border_radius="24px",
        border=f"1px solid {BORDER_COLOR}",
        background="linear-gradient(135deg, rgba(8, 247, 163, 0.03), rgba(3, 9, 21, 0.4))",
